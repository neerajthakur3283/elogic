  ...
  return (
    <>
        {infoStepData.map((data,index)=>(
            <div className={`2xl:px-[80px] xl:px-[60px] lg:px-[53px] md:px-[35px] px-[20px] 2xl:py-[40px] xl:py-[30px] md:py-[26px] py-[20px] flex justify-between items-start 2xl:rounded-[15px] xl:rounded-[12px] md:rounded-[11px] rounded-[10px] ${data.bgCard}`} key={index}>
                <div className=''>
                ...
                <div className='2xl:w-[651px] xl:w-[490px] lg:w-[435px]'>
                    <ul className='ps-[17px] list-disc grid grid-cols-2 2xl:gap-5 xl:gap-4 md:gap-3 gap-2.5'>
                        {data.cardListPoints.map((item,subindex)=>(
                            <li className={`2xl:text-[18px] xl:text-[13.5px] md:text-[12px] ${item.pointColor}`} key={subindex}>
                                {item.pointText}
                            </li>
                        ))}
                    </ul>
