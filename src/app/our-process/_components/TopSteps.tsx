  ...
  return (
    <>
        {firstStepData.map((data,index)=>(
            <div className={`justify-center flex flex-col items-center ${data.colsreverses}`} key={index}>
                ...
                <div className='2xl:py-3.5 py-2.5'>
                    <Image src={data.stepIcon} alt='' />
                </div>
                <h6 className={`2xl:text-[18px] xl:text-[13.5px] md:text-[12px] font-semibold ${data.titleColor}`}>{data.title}</h6>
                <p className={`2xl:pt-2.5 pt-2 2xl:text-[18px] xl:text-[13.5px] md:text-[12px] text-center ${data.preColor}`}>{data.stepDes}</p>
            </div>
        ))}
    </>
  )
